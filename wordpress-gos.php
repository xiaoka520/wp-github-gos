...
function github_read_dir_queue($dir)
{
	...
	// 广度优先遍历目录
	while (!empty($queue)) {
		$path = array_shift($queue); // 弹出队列头部元素
		...
		if (is_dir($path) && $handle = opendir($path)) {
			while (($file = readdir($handle)) !== false) {
				...
				if (is_dir($real_path)) {
					$queue[] = $real_path; // 将子目录加入队列尾部
				}
			}
			closedir($handle); // 关闭当前目录句柄
		}
	}
	...
	foreach ($files as $i => $file) {
		if (!is_dir($file)) {
			$dd[$i]['j'] = $file;
			$dd[$i]['x'] = '/' . get_option('upload_path') . explode(get_option('upload_path'), $file)[1];
		}
	}
	...
	<div class="wrap" style="margin: 10px;">
		...
		<form name="form1" method="post" action="<?php echo wp_nonce_url('./options-general.php?page=' . GITHUB_SYNC_BASEFOLDER . '/wordpress-gos.php'); ?>">
			<table class="form-table">
				<tr>
					<th>
						<legend><?php _e('用户名', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('仓库名', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('不上传缩略图', 'wordpress-gos') ?></legend>
					</th>
					<td><input type="checkbox" name="nothumb" <?php
						...
						<p><?php _e('建议不勾选', 'wordpress-gos') ?></p>
					</td>
				</tr>
				<tr>
					<th>
						<legend><?php _e('不在本地保留备份', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('本地文件夹', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('URL前缀', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('URL参数', 'wordpress-gos') ?></legend>
					</th>
				...
				<tr>
					<th>
						<legend><?php _e('更新选项', 'wordpress-gos') ?></legend>
					</th>
		...
		<form name="form1" method="post" action="<?php echo wp_nonce_url('./options-general.php?page=' . GITHUB_SYNC_BASEFOLDER . '/wordpress-gos.php'); ?>">
			<table class="form-table">
				<tr>
					<th>
						<legend><?php _e('同步历史附件', 'wordpress-gos') ?></legend>
					</th>
